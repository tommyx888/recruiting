...
function initSupabase() {
    if (!supabaseInstance) {
        try {
            // Import config dynamically to avoid issues with module loading
...
async function addCandidate(e) {
    ...
    const validation = window.utils.validateRequiredFields({
        name, department, position, source, date_obtained
    }, requiredFields);
    ...
    try {
        ...
        if (cvFile) {
            const cvValidation = window.utils.validateFile(cvFile);
            if (!cvValidation.isValid) {
                window.utils.showMessage(`CV upload error: ${cvValidation.message}`, 'error');
                return;
            }
        ...
        // Handle Assessment Form upload with validation
        const assessmentFile = document.getElementById('assessment').files[0];
        if (assessmentFile) {
            const assessmentValidation = window.utils.validateFile(assessmentFile);
            if (!assessmentValidation.isValid) {
                window.utils.showMessage(`Assessment upload error: ${assessmentValidation.message}`, 'error');
                return;
            }
        ...
        window.utils.showMessage('Candidate added successfully!', 'success');
        showCandidates();
    } catch (error) {
        console.error('Error adding candidate:', error);
        window.utils.showMessage('Error adding candidate: ' + error.message, 'error');
    }
}
